...
def find_all_snapshots(universe: str) -> list[Path]:
    ...
    for p in DATA_DIR.glob("*/*.csv"):
        ...
        if not m or m.group("universe") != universe:
            continue
...
def load_snapshot(path: Path) -> pd.DataFrame:
    ...
    df["bm"] = pd.to_numeric(df.get("bm"), errors="coerce") if "bm" in df.columns else np.nan
...
def clean_df(df: pd.DataFrame) -> pd.DataFrame:
    return df[df["ticker"].notna() & df["date"].notna() & df["mktcap"].notna() & df["ICC"].notna() & np.isfinite(df["mktcap"]) & np.isfinite(df["ICC"]) & (df["mktcap"] > 0)].copy()

def build_market_history(paths: list[Path]) -> pd.DataFrame:
    rows = []
    for path in paths:
    ...
    return pd.DataFrame(rows).sort_values("date").reset_index(drop=True) if rows else pd.DataFrame(columns=["date", "vw_icc", "ew_icc", "n_firms", "source_file"])

def build_value_history(paths: list[Path]) -> pd.DataFrame:
    rows = []
    for path in paths:
        df = load_snapshot(path)
        df = df[df["date"].notna() & df["mktcap"].notna() & df["bm"].notna() & df["ICC"].notna() & np.isfinite(df["mktcap"]) & np.isfinite(df["bm"]) & np.isfinite(df["ICC"]) & (df["mktcap"] > 0) & (df["bm"] > 0)].copy()
        if df.empty:
            continue
        ...
        pt = df.groupby("portfolio", dropna=False).apply(lambda g: pd.Series({"vw_icc": weighted_mean(g["ICC"], g["mktcap"]), "n_firms": int(len(g))})).reset_index()
        d = dict(zip(pt["portfolio"], pt["vw_icc"]))
        icc_sl, icc_bl, icc_sh, icc_bh = d.get("S/L", np.nan), d.get("B/L", np.nan), d.get("S/H", np.nan), d.get("B/H", np.nan)
        ...
        rows.append({"date": str(df["date"].iloc[0]), "value_icc": value_icc, "growth_icc": growth_icc, "ivp_bm": ivp_bm, "n_firms": int(len(df)), "source_file": str(path.relative_to(REPO))})
    return pd.DataFrame(rows).sort_values("date").reset_index(drop=True) if rows else pd.DataFrame(columns=["date", "value_icc", "growth_icc", "ivp_bm", "n_firms", "source_file"])
...
def build_latest_industry(latest_usall: pd.DataFrame) -> pd.DataFrame:
    ...
    return df.groupby("sector", dropna=False).apply(lambda g: pd.Series({"vw_icc": weighted_mean(g["ICC"], g["mktcap"]), "ew_icc": float(g["ICC"].mean()), "n_firms": int(len(g)), "total_mktcap": float(g["mktcap"].sum())})).reset_index().sort_values(["total_mktcap", "sector"], ascending=[False, True]).reset_index(drop=True)
...
def build_holdings_proxy(rows_cfg: pd.DataFrame, latest_usall: pd.DataFrame, kind: str) -> pd.DataFrame:
    ...
    for _, cfg in rows_cfg.iterrows():
        ...
        try:
            ...
            if matched.sum() == 0:
                out_rows.append({"ticker": ticker, "label": label, "category": category, "country": country, "vw_icc": np.nan, "coverage_weight": 0.0, "n_holdings": int(len(h)), "n_matched": 0, "holding_source": "yfinance_top_holdings", "status": "no_matched_holdings"})
                continue
            icc = weighted_mean(merged.loc[matched, "ICC"], merged.loc[matched, "weight"])
            out_rows.append({"ticker": ticker, "label": label, "category": category, "country": country, "vw_icc": icc, "coverage_weight": coverage, "n_holdings": int(len(h)), "n_matched": int(matched.sum()), "holding_source": "yfinance_top_holdings", "status": "ok" if coverage > 0 else "low_coverage"})
        except Exception as e:
            out_rows.append({"ticker": ticker, "label": label, "category": category, "country": country, "vw_icc": np.nan, "coverage_weight": np.nan, "n_holdings": np.nan, "n_matched": np.nan, "holding_source": "yfinance_top_holdings", "status": f"error: {type(e).__name__}"})
...
def main() -> None:
    ...
    usall_paths = find_all_snapshots(args.universe)
    if not usall_paths:
        raise FileNotFoundError("No usall snapshot found under data/YYYYMM/")
    latest_usall_path = usall_paths[-1]
    latest_usall = load_snapshot(latest_usall_path)
    latest_date = str(latest_usall["date"].dropna().iloc[0])
    market_history = build_market_history(usall_paths)
    value_history = build_value_history(usall_paths)
